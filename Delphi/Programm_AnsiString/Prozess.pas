...
TProcesses = class(TObject)
  private
     Dictionary: TDictionary<AnsiString, AnsiString>;

  public
     constructor Create;
     destructor Destroy; override;

     function LoadCatastre(filename : AnsiString) : Int64;
     ...
     function WriteToFile(fileName : AnsiString) : Int64;

     function DeleteDirectories(path : AnsiString) : Int64;
     function WriteToDir(path : AnsiString) : Int64;
     function ReadFromDir(path : AnsiString) : Int64;
     function FreeData : Int64;
     function Partition : Int64;
     function SortPartition : Int64;
     function WritePartOfCatastre(fileName : AnsiString; from_param, to_param : Int64) : int64;

  private
     function FindExtension(strText : AnsiString) : Integer;
     function MyCompareText(Left, Right : Ansistring) : Integer;
...
begin
   Dictionary := TDictionary<AnsiString, AnsiString>.Create;
   Dictionary.Add('ä','ae');
   Dictionary.Add('Ä','Ae');
   Dictionary.Add('ö','oe');
   Dictionary.Add('Ö','Oe');
...
function TProcesses.LoadCatastre(filename : AnsiString) : Int64;
var
   text : TStringList;
   row  : TStringList;
   fmt  : TFormatSettings;
         ...
         try
            ...
            begin
               ...
               begin
                  ...
                  mCadastre[i].City         := AnsiString(row[0]);
				          mCadastre[i].Street       := AnsiString(row[1]);
				          mCadastre[i].StreetNumber := AnsiString(AnsiLowerCase(row[2]));
				          mCadastre[i].ZipCode      := AnsiString(row[3]);
				          mCadastre[i].Urban_Unit   := AnsiString(row[4]);
				          mCadastre[i].Old_Unit     := AnsiString(row[5]);
				          mCadastre[i].District     := AnsiString(row[6]);
...
function TProcesses.FindExtension(strText : AnsiString) : Integer;
var
   i : Integer;
begin
   for i := Length(strText)  downto 1 do
   begin
      if strText[i] in ['0' .. '9'] then
      begin
         Result := i;
         exit;
      end;
...
function TProcesses.MyCompareText(Left, Right : Ansistring) : Integer;
  function Replace(text : AnsiString) : AnsiString;
  var
     pattern: AnsiString;
  begin
     for pattern in Dictionary.Keys do
        if AnsiContainsStr(text, pattern) then
            ...
            begin
               ...
               begin
                  ...
                  begin
                     ...
                     begin
                        n1 := StrToInt(String(AnsiLeftStr(Left.StreetNumber, h1)));
                        n2 := StrToInt(String(AnsiLeftStr(Right.StreetNumber, h2)));
                        cmp := CompareValue(n1, n2);
                        if cmp <> 0 then Result := cmp
                        else
                        begin
                           z1 := AnsiRightStr(Left.StreetNumber, Length(Left.StreetNumber) - h1);
                           z2 := AnsiRightStr(Right.StreetNumber, Length(Right.StreetNumber) - h2);
...
function TProcesses.WriteToFile(fileName : AnsiString) : Int64;
var
   text : TextFile;
   i : Int64;
begin
...
function TProcesses.DeleteDirectories(path : AnsiString) : Int64;
var
...
function TProcesses.WriteToDir(path : AnsiString) : Int64;
   ...
   function open_file(var text : TextFile; filename : AnsiString) : Boolean;
   begin
      try
...
var
   strCity, strDistrict, strUrban_Unit, strStreet : AnsiString;
   strNewPath, strFile : AnsiString;
   text : TextFile;
   is_open : Boolean;
   i : Int64;
   fmt  : TFormatSettings;
      ...
      begin
         ...
         begin
            ...
            begin
               strStreet := mCadastre[i].Street;
               //j := 0;
               strFile   := strNewPath + AnsiString('\') + strStreet + AnsiString('.csv');
               if is_open then is_open := close_file(text);
               is_open := open_file(text, strFile);
            end;
...
function TProcesses.ReadFromDir(path : AnsiString) : Int64;
   function GetItems(path : AnsiString; params : array of AnsiString; boDirectory : Boolean) : TStringList;
   var
      SearchFunc : TSearchRec;
      compl_path : AnsiString;
      FileAttrs : Integer;
      i : Integer;
   begin
      ...
      begin
         compl_path := AnsiString(TPath.Combine(String(compl_path), String(params[i])));
      end;
   ...
   function ReadItems(path, city, urban_unit, district, street : AnsiString) : Int64;
   var
     text        : TStringList;
     row         : TStringList;
     filename    : String;
               ...
               begin
                  ...
                  begin
                     ...
                     mCadastre[old + i].City         := city;
				             ...
				             mCadastre[old + i].StreetNumber := AnsiString(AnsiLowerCase(row[0]));
				             mCadastre[old + i].ZipCode      := AnsiString(row[1]);
				             mCadastre[old + i].Urban_Unit   := urban_unit;
				             mCadastre[old + i].Old_Unit     := AnsiString(row[2]);
...
var
  ...
  strCity, strUrban_Unit, strDistrict, strStreet : AnsiString;
  h, i, j : Integer;
  k: Integer;
begin
...
function TProcesses.WritePartOfCatastre(fileName : AnsiString; from_param, to_param : Int64) : int64;
var
   text : TextFile;
   i : Int64;
begin
