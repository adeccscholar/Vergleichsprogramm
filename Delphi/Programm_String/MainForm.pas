...
begin
   Memo1.Lines.Clear;
   dsStart := Now;
   cnt := Proc.LoadCatastre(Edit1.Text + 'berlin_infos.dat');
   dsEnd  := Now;
...
begin
   dsStart := Now;
   cnt := Proc.WriteToFile(Edit1.Text + 'output_streets.txt');
   dsEnd  := Now;
	 time := MilliSecondsBetween(dsEnd, dsStart);
...
begin
   dsStart := Now;
   Proc.DeleteDirectories(Edit1.Text);
   dsEnd  := Now;
	 time := MilliSecondsBetween(dsEnd, dsStart);
...
begin
   dsStart := Now;
   cnt := Proc.WriteToDir(Edit1.Text);
   dsEnd  := Now;
	 time := MilliSecondsBetween(dsEnd, dsStart);
...
begin
   dsStart := Now;
   cnt := Proc.ReadFromDir(Edit1.Text);
   dsEnd  := Now;
	 time := MilliSecondsBetween(dsEnd, dsStart);
...
begin
   ...
   dsStart := Now;
   cnt := Proc.WritePartOfCatastre(Edit1.Text + 'output_distance.txt', 0, cnt);
   dsEnd  := Now;
	 time := MilliSecondsBetween(dsEnd, dsStart);
